...
<title>故事分类--><c:if test="${edit}">编辑</c:if>  <c:if test="${!edit}">新增</c:if></title>
<style type="text/css">
		...
		<div class="content-wrapper">
			...
			<div class="box box-info">
	            <div class="box-header with-border">
	              <h3 class="box-title">故事分类--><c:if test="${edit}">编辑</c:if>  <c:if test="${!edit}">新增</c:if></h3>
	            </div>
	            <!-- /.box-header -->
	            <form id="categoryForm" enctype="multipart/form-data" method="POST" action="${base}/admin/auth/category/save" >
	              <div class="box-body">
	                <!-- text input -->
	                <div class="form-group">
	                  <label>名称</label>
	                  <input type="hidden" id="id" name="id" value="${category.id }">
	                  <input type="text" id="name" name="name" class="form-control" value="${category.name }" placeholder="6-20个字符">
	                ...
	                <div id ="divPreview" class="form-group">
	                	<img alt="预览图片" src="${category.imgUrl }" id="imgPreview" style="max-width:100%;height: 50px;width: 100px;">
	                </div>
	                <!-- textarea -->
	                <div class="form-group">
	                  <label>序号</label> 
	                  <input type="text" maxlength="3"  class="form-control" placeholder="0-999之间整数" id="seqNum" name="seqNum" value="${category.seqNum }" >
	                  <span class="help-block"></span>
	                </div>
